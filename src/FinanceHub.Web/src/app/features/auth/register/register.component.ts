    ...
    <div class="auth-container">
      <mat-card class="auth-card">
        <div class="auth-header">
          ...
          <h1>Criar Conta</h1>
          <p>Comece a controlar suas financas</p>
        </div>
        <mat-card-content>
          <form [formGroup]="form" (ngSubmit)="onSubmit()">
            ...
            <button mat-raised-button color="primary" class="full-width submit-btn" type="submit" [disabled]="loading">
              @if (loading) { <mat-spinner diameter="20"></mat-spinner> } @else { Cadastrar }
            </button>
          </form>
        </mat-card-content>
        <mat-card-actions>
          <p style="text-align:center;">Ja tem conta? <a routerLink="/login" class="link">Entrar</a></p>
        </mat-card-actions>
      </mat-card>
    </div>
  `,
...
export class RegisterComponent {
  ...
  onSubmit(): void {
    ...
    this.authService.register(email, fullName, password).subscribe({
      next: () => { this.router.navigate(['/dashboard']); this.snackBar.open('Conta criada com sucesso!', 'Fechar', { duration: 3000 }); },
